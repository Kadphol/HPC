...
* 60070501004 Kadphol Lenlerdphol
* 60070503415 Chawakorn Boonrin
*/
...
int main(int argc, char *argv[]) 
{
    ...
    if(rank == 0) 
    {
        ...
        {  
            ...
            if(size > 1) 
            {
                ...
                for(int count = 1;count < size;count++) 
                {
                    MPI_Irecv(&matrixC[partRow * count][0],partRow * colA,MPI_FLOAT,count,5,MPI_COMM_WORLD,&request[0]);
    ...
    else
    {
        int i,j;
        int partRow,col;
        MPI_Irecv(&partRow,1,MPI_INT,0,1,MPI_COMM_WORLD,&request[1]);
        MPI_Irecv(&col,1,MPI_INT,0,2,MPI_COMM_WORLD,&request[2]);
        ...
        MPI_Irecv(&matrixA[0][0],partRow * col,MPI_FLOAT,0,3,MPI_COMM_WORLD,&request[3]);
        MPI_Irecv(&matrixB[0][0],partRow * col,MPI_FLOAT,0,4,MPI_COMM_WORLD,&request[4]);
